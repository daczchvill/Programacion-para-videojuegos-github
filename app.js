...
window.abrirFormMenu = function() {
  ...
  const btnEliminar = document.getElementById('menu-foto-eliminar-btn');
  if (btnEliminar) btnEliminar.style.display = 'none';
  const btnDescargar = document.getElementById('menu-foto-descargar-btn');
...
window.editarMenuItem = function(id) {
  ...
  // Mostrar/ocultar botones según haya imagen
  const btnEliminar = document.getElementById('menu-foto-eliminar-btn');
  if (btnEliminar) btnEliminar.style.display = p.imagen_url ? 'block' : 'none';
  const btnDescargar = document.getElementById('menu-foto-descargar-btn');
  if (btnDescargar) btnDescargar.style.display = p.imagen_url ? 'block' : 'none';
  document.getElementById('menu-form-container').style.display = 'block';
  document.getElementById('menu-form-container').scrollIntoView({ behavior:'smooth', block:'nearest' });
};

window.descargarFotoMenu = async function() {
  const urlActual = document.getElementById('menuImagenUrl').value.trim();
  if (!urlActual) {
    toast('No hay foto para descargar', 'error');
    return;
  }

  showSync(true);
  try {
    // Obtener nombre del producto para el archivo descargado
    const nombreProducto = (document.getElementById('menuNombre').value || 'producto').trim();
    ...
    // Extraer extensión del URL original
    const extMatch = urlActual.match(/\.([a-z]+)(?:\?|$)/i);
    const ext = extMatch ? extMatch[1] : 'jpg';

    // Descargar la imagen como blob
    const response = await fetch(urlActual);
    if (!response.ok) throw new Error('No se pudo descargar la foto (HTTP ' + response.status + ')');
    const blob = await response.blob();

    // Crear link de descarga y disparar click
    const blobUrl = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = blobUrl;
    a.download = `taco_parado_${nombreLimpio}.${ext}`;
...
window.eliminarFotoMenu = async function() {
  const urlActual = document.getElementById('menuImagenUrl').value.trim();
  if (!urlActual) return;

  if (!confirm('¿Quitar la foto de este producto?\n\nLa foto se eliminará permanentemente del almacenamiento.')) return;

  showSync(true);
  try {
    // Intentar eliminar del bucket de storage
    // URL pública tiene formato: .../storage/v1/object/public/menu/producto_123.jpg
    const match = urlActual.match(/\/menu\/([^?]+)/);
    if (match && match[1]) {
      const filename = decodeURIComponent(match[1]);
      const { error: delErr } = await window.supabase.storage.from('menu').remove([filename]);
      if (delErr) {
        console.warn('No se pudo borrar el archivo del storage:', delErr);
      }
    }

    // Limpiar el campo (la BD se actualiza al guardar el producto)
    document.getElementById('menuImagenUrl').value = '';
    document.getElementById('menu-foto-preview').innerHTML = '<span style="opacity:0.5; font-size:12px;">Sin foto</span>';
    document.getElementById('menu-foto-eliminar-btn').style.display = 'none';
    const btnDescargar = document.getElementById('menu-foto-descargar-btn');
    if (btnDescargar) btnDescargar.style.display = 'none';

    // Limpiar input de archivo por si se quiere subir otro
    const fileInput = document.getElementById('menu-foto-input');
    if (fileInput) fileInput.value = '';

    toast('✓ Foto removida. Guarda el producto para confirmar.');
  } catch (err) {
...
window.subirFotoMenu = async function(event) {
  ...
  try {
    ...
    // Mostrar botones eliminar y descargar
    const btnEliminar = document.getElementById('menu-foto-eliminar-btn');
...
window.cargarReporte = async function() {
  const rango = document.getElementById('reporteRangoSelect').value;
  const sedeId = document.getElementById('reporteSedeSelect').value || null;
  ...
  kpisEl.innerHTML = `
    <div style="text-align: center; padding: 20px; color: var(--text-soft); grid-column: 1/-1;">
  ...
  try {
    const { data, error } = await window.supabase.rpc('reporte_ventas_rango', {
    ...
    console.error(err);
    kpisEl.innerHTML = `
      <div style="text-align: center; padding: 20px; color: var(--red); grid-column: 1/-1;">
        Error: ${err.message}
      </div>
    `;
  }
...
function renderReporte(data) {
  ...
  const ventas = parseFloat(kpis.ventas_totales) || 0;
  const pedidos = kpis.total_pedidos || 0;
  const ticket = parseFloat(kpis.ticket_promedio) || 0;
  const dias = kpis.dias_con_ventas || 0;
  const promedioDiario = dias > 0 ? ventas / dias : 0;

  // KPIs
  document.getElementById('reporteKpis').innerHTML = `
    <div style="background: #fff; border: 3px solid var(--ink); border-radius: 12px; padding: 14px; text-align: center; box-shadow: 0 3px 0 var(--ink);">
      <div style="font-size: 26px;">💰</div>
      <div style="font-family: 'Bungee', sans-serif; font-size: 18px; color: var(--green, #16a34a); margin-top: 4px;">
        ${fmtMoney(ventas)}
      </div>
      <div style="font-size: 10px; color: var(--text-soft); text-transform: uppercase; letter-spacing: 1px; margin-top: 2px;">Ventas Totales</div>
    </div>
    <div style="background: #fff; border: 3px solid var(--ink); border-radius: 12px; padding: 14px; text-align: center; box-shadow: 0 3px 0 var(--ink);">
      <div style="font-size: 26px;">🛒</div>
      <div style="font-family: 'Bungee', sans-serif; font-size: 22px; color: var(--tp-orange); margin-top: 4px;">
        ${pedidos}
      </div>
      <div style="font-size: 10px; color: var(--text-soft); text-transform: uppercase; letter-spacing: 1px; margin-top: 2px;">Pedidos</div>
    </div>
    <div style="background: #fff; border: 3px solid var(--ink); border-radius: 12px; padding: 14px; text-align: center; box-shadow: 0 3px 0 var(--ink);">
      <div style="font-size: 26px;">📊</div>
      <div style="font-family: 'Bungee', sans-serif; font-size: 18px; color: var(--tp-deep-blue); margin-top: 4px;">
        ${fmtMoney(ticket)}
      </div>
      <div style="font-size: 10px; color: var(--text-soft); text-transform: uppercase; letter-spacing: 1px; margin-top: 2px;">Ticket Promedio</div>
    </div>
    <div style="background: #fff; border: 3px solid var(--ink); border-radius: 12px; padding: 14px; text-align: center; box-shadow: 0 3px 0 var(--ink);">
      <div style="font-size: 26px;">📅</div>
      <div style="font-family: 'Bungee', sans-serif; font-size: 18px; color: var(--ink); margin-top: 4px;">
        ${fmtMoney(promedioDiario)}
      </div>
      <div style="font-size: 10px; color: var(--text-soft); text-transform: uppercase; letter-spacing: 1px; margin-top: 2px;">Promedio Diario</div>
    </div>
  `;
  ...
  if (porDia.length > 0) {
    document.getElementById('reporteGraficoWrap').style.display = 'block';
    renderGraficoVentas(porDia);
  } else {
    document.getElementById('reporteGraficoWrap').style.display = 'none';
  }

  // Top productos
  const topProds = data.top_productos || [];
  if (topProds.length > 0) {
    document.getElementById('reporteTopWrap').style.display = 'block';
    const maxUds = Math.max(...topProds.map(p => p.uds));
    document.getElementById('reporteTop').innerHTML = topProds.map((p, i) => {
      const pct = (p.uds / maxUds) * 100;
      const medallas = ['🥇', '🥈', '🥉'];
      return `
        <div style="margin-bottom: 8px; display: grid; grid-template-columns: 30px 1fr auto; gap: 10px; align-items: center;">
          <span style="font-size: 18px;">${medallas[i] || (i+1)}</span>
          <div>
            <div style="font-weight: 800; font-size: 13px;">${escapeHtml(p.nombre)}</div>
            <div style="background: #eee; height: 8px; border-radius: 4px; margin-top: 4px; overflow: hidden;">
              <div style="background: var(--tp-orange); height: 100%; width: ${pct}%; border-radius: 4px;"></div>
            </div>
          </div>
          <div style="text-align: right;">
            <div style="font-weight: 900; font-family: 'JetBrains Mono', monospace;">${p.uds} uds</div>
            <div style="font-size: 10px; color: var(--text-soft);">${fmtMoney(parseFloat(p.total))}</div>
          </div>
        </div>
      `;
    }).join('');
  } else {
    document.getElementById('reporteTopWrap').style.display = 'none';
  }

  // Tabla por día
  if (porDia.length > 0) {
    document.getElementById('reporteTablaWrap').style.display = 'block';
    document.getElementById('reporteTablaBody').innerHTML = porDia.map(d => {
      const ticket = d.pedidos > 0 ? parseFloat(d.ventas) / d.pedidos : 0;
      return `
        <tr style="border-bottom: 1px solid #eee;">
          <td style="padding: 8px;">${new Date(d.dia).toLocaleDateString('es-CO', {weekday:'short', day:'numeric', month:'short'})}</td>
          <td style="padding: 8px; text-align: right; font-weight: 700;">${d.pedidos}</td>
          <td style="padding: 8px; text-align: right; font-weight: 800; color: var(--green, #16a34a);">${fmtMoney(parseFloat(d.ventas))}</td>
          <td style="padding: 8px; text-align: right;">${fmtMoney(ticket)}</td>
          <td style="padding: 8px; text-align: right; font-family: 'JetBrains Mono', monospace; font-size: 11px;">${fmtMoney(parseFloat(d.efectivo))}</td>
          <td style="padding: 8px; text-align: right; font-family: 'JetBrains Mono', monospace; font-size: 11px;">${fmtMoney(parseFloat(d.datafono))}</td>
          <td style="padding: 8px; text-align: right; font-family: 'JetBrains Mono', monospace; font-size: 11px;">${fmtMoney(parseFloat(d.transferencia))}</td>
        </tr>
      `;
    }).join('');
  } else {
    document.getElementById('reporteTablaWrap').style.display = 'none';
  }
}

function renderGraficoVentas(porDia) {
  const cont = document.getElementById('reporteGrafico');
  const maxVenta = Math.max(...porDia.map(d => parseFloat(d.ventas) || 0));
  if (maxVenta === 0) { cont.innerHTML = '<div style="text-align:center; color: var(--text-soft);">Sin ventas</div>'; return; }
  ...
  const barW = chartW / porDia.length;
  ...
  for (let i = 0; i <= 4; i++) {
    ...
    const val = maxVenta * (1 - i / 4);
    svg += `<line x1="${padLeft}" y1="${y}" x2="${w-padRight}" y2="${y}" stroke="#eee" />`;
    svg += `<text x="${padLeft - 6}" y="${y + 4}" text-anchor="end" font-size="9" fill="#999" font-family="monospace">${fmt(Math.round(val))}</text>`;
  }
  
  // Barras
  porDia.forEach((d, i) => {
    const v = parseFloat(d.ventas) || 0;
    const barH = (v / maxVenta) * chartH;
    const x = padLeft + i * barW + barW * 0.15;
    const y = padTop + chartH - barH;
    const wBar = barW * 0.7;
    
    svg += `<rect x="${x}" y="${y}" width="${wBar}" height="${barH}" fill="#FF9000" stroke="#0d2451" stroke-width="1.5" rx="3"></rect>`;
    
    // Label dia
    const fecha = new Date(d.dia);
    const lbl = fecha.toLocaleDateString('es-CO', {day:'numeric', month:'numeric'});
    svg += `<text x="${x + wBar/2}" y="${h - 12}" text-anchor="middle" font-size="10" fill="#0d2451" font-weight="700">${lbl}</text>`;
    
    // Valor encima
    if (barH > 20) {
      svg += `<text x="${x + wBar/2}" y="${y - 4}" text-anchor="middle" font-size="9" fill="#0d2451" font-weight="800">${fmt(Math.round(v/1000))}k</text>`;
    }
  });
...
// Cargar sedes en select del reporte
async function cargarSedesReporte() {
  const sel = document.getElementById('reporteSedeSelect');
  if (!sel || sel.dataset.cargado) return;
  try {
    const { data } = await window.supabase.from('sedes').select('id, nombre').eq('activa', true);
    if (data) {
      data.forEach(s => {
...
// Auto-cargar reporte al cambiar tab
document.addEventListener('click', e => {
  if (e.target.matches('[data-tab="reportes"]')) {
    cargarSedesReporte();
    setTimeout(() => cargarReporte(), 200);
  }
});
